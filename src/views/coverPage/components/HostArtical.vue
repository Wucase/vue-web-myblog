<template>
  <div class="hostArt"  id="slider">
    <el-row class="animated articalTitle" v-if="animatedClass" :class="{fadeInDown: animatedClass}">
      <el-col :span="24">
        <div class="infoTitle">
          {{infoTitle.title}}
        </div>
        <div class="infoTip"></div>
        <div class="infoMsg">{{infoTitle.infoMsgOne}}<br>{{infoTitle.infoMsgTwo}}</div>
      </el-col>
    </el-row>
    <el-row  v-if="animatedArtClass">
      <template>
        <el-col :span="8" :class="{fadeInUp:animatedArtClass}" class="animated articalLists"
          v-for="(item, index) in hostArtical" :key="index">
          <div class="articalList">
            ...
            <div class="articalMsg">
              ...
              <p class="articalMsgFont">FastReport.Net是一款适用于Windows Forms, ASP.NET和MVC框架的功能齐全的报表分析解决方案，支持.Net
                Framework
                2.0到4.x。</p>
